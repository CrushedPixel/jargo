...
The following data types are supported for id fields:
- primitive integer types (`int`, `int64`, `uint`, ...)
- `UUID` types
- `string`
- All types implementing [`encoding.TextMarshaler`][encoding.TextMarshaler] and [`encoding.TextUnmarshaler`][encoding.TextUnmarshaler]

## [Auto-incrementing ids](#auto-incrementing-ids)
All integer types are **auto-incrementing**:
~~~go
type User struct {
    Id int64
}

// insert User with Id set to zero value
res, err := req.Resource().InsertInstance(req.DB(), &User{}).Result()
if err != nil {
...
// user ids are automatically set by the database
log.Printf("first user: %d, second user: %d", user1.Id, user2.Id)
~~~

## [UUID Ids](#uuid-ids)
UUIDs are auto-generated by the database using the [uuid-ossp][uuid-ossp] extension.
~~~go
type User struct {
    Id *uuid.UUID // many different UUID libraries are supported
}
...
log.Printf("user uuid: %s", user.Id)
~~~

## [String Ids](#string-ids)
For other types, the id field **must be set to a unique value** before inserting a resource into the database.
~~~go
type User struct {
    Id string
}
...
if err != nil {
    ...
    // as an emtpy string already exists as primary key
    return jargo.NewErrorResponse(err)
}
~~~

Data types implementing the `TextMarshaler` and `TextUnmarshaler` interfaces 
are converted into their textual representation for the exposed JSON API.
